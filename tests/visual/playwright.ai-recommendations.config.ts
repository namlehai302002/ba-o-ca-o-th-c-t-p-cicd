...
if (!baseURL) throw new Error('WMS_BASE_URL is required for AI-3 recommendation verification.');
if (!storageState) throw new Error('WMS_AUTH_STATE or the default authenticated state is required.');

export default defineConfig({
  testDir: '.',
  testMatch: 'wms-ai-recommendations.spec.ts',
  timeout: 120_000,
  fullyParallel: false,
  workers: 1,
  use: {
  ...
  outputDir: '../../artifacts/ai-smart-cycle-count/AI3/playwright-results',
  projects: [
    {
  ...
  reporter: [
    ['list'],
    ['json', { outputFile: '../../artifacts/ai-smart-cycle-count/AI3/playwright-report.json' }],
    ['html', { outputFolder: '../../artifacts/ai-smart-cycle-count/AI3/playwright-report', open: 'never' }]
  ]
});
